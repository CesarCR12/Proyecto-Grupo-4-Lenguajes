...
BEGIN
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Se agregó el producto');
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error al agregar');
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Se canceló la agregación del producto');
...
BEGIN
    EXECUTE IMMEDIATE
        'SELECT SUM(PRECIO) FROM FIDE_INVENTARIO_TB WHERE FIDE_INVENTARIO_TB_ID_INVENTARIO_PK IN (' || P_IDS_INVENTARIO || ')'
        INTO V_SUBTOTAL;
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('La factura con el id ' || P_ID_FACTURA || ' es de ' || P_TOTAL_VENTA);
EXCEPTION
    WHEN OTHERS THEN
